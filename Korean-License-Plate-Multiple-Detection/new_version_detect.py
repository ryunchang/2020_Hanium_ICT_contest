...
wf=img_color.copy()
copy1_img=img_color.copy()
gray = cv2.cvtColor(img_color, cv2.COLOR_BGR2GRAY)
blur = cv2.GaussianBlur(gray, (5,5), 3)
canny = cv2.Canny(blur, 100, 100)
contours,hierarchy = cv2.findContours(canny, mode=cv2.RETR_TREE, method=cv2.CHAIN_APPROX_NONE)
...
# 번호판 전체 크기를 고려하여 설정하였다. 거리에 맞추어 설정하자/
NEW_CAR_ratio=6
OLD_CAR_ratio=2
MAX_plate_area=10000
MIN_plate_area=3000
...
for i in range(len(contours)):
    ...
    cv2.rectangle(wf,(x,y),(x+w, y+h), (0, 255,0), 1)
    ...
    if (aspect_ratio1 >= NUM_MIN_RATIO) and (aspect_ratio1<=NUM_MAX_RATIO) and (rect_area >=NUM_MIN_AREA) :
        cv2.rectangle(copy_img,(x,y),(x+w, y+h), (0, 255,0), 1)
        box2.append(cv2.boundingRect(cnt))


cv2.imshow("show image",wf)
cv2.imwrite("boxidfmg.jpg",wf)
cv2.waitKey(0)

cv2.imshow("show image",img_color)
cv2.imwrite("boximg.jpg",img_color)
cv2.waitKey(0)

cv2.imshow("show image",copy_img)
cv2.imwrite("one nim.jpg",copy_img)
cv2.waitKey(0)
...
#번호판 인식과정 box1에서 box2에 있는게 5개 정도 포함이 되는 경우 번호판이라 칭함
select=[]
for i in range(len(box1)):
    count=0
...
for i in range(len(select)):
    number_plate[i]=(copy1_img[box1[select[i]][1]        :     box1[select[i]][3]+      box1[select[i]][1]  ,        box1[select[i]][0]   :  box1[select[i]][0]  + box1[select[i]][2]                    ])
    cv2.imshow("show",number_plate[i])
    cv2.waitKey(0)


for i in range(len(select)):
    cv2.imwrite("plate" +str(i)+".jpg",number_plate[i])
