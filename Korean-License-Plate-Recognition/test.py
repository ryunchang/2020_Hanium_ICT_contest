...
def detection(img_color) :

    #img_color = cv2.imread('1234.jpg', cv2.IMREAD_COLOR)
    img_colord = cv2.resize(img_color, dsize=(2688,1520), interpolation=cv2.INTER_AREA)

    copy_img=img_color.copy()
    img_gray = cv2.cvtColor(img_colord, cv2.COLOR_BGR2GRAY)
...
if __name__ == '__main__':
    ...
    cap = cv2.VideoCapture("rtsp://admin:12341234!@192.168.1.108:554/") 
    ret, img_color = cap.read()
    cv2.imshow("h", img_color)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    first, second, third = detection(img_color) 
