...
def detection():
    ...
    NEW_CAR_ratio=9
    OLD_CAR_ratio=2.5
    MAX_plate_area=height*width*0.006
    MIN_plate_area=1000
    ...
    for i in range(len(valid_plate)):
        if  plate_box[valid_plate[i]][0]<width*0.5:
            if cnt[0]==0:
                valid_plate_temp[0] = valid_plate[i]
                cnt[0] = 1
        
        elif plate_box[valid_plate[i]][0]<width*0.76:        
            if cnt[1]==0:
    ...
    for i in range(len(cnt)):
        if not (cnt[i] == 0) :
            number_plate[i+1] =  (copy_img[plate_box[valid_plate[i]][1]-10 : plate_box[valid_plate[i]][3]+ plate_box[valid_plate[i]][1]+10, plate_box[valid_plate[i]][0]-10 :  10+plate_box[valid_plate[i]][0]  + plate_box[valid_plate[i]][2] ])
